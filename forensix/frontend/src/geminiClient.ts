export const GEMINI_KEYS = [
  'AIzaSyAxt7Qe717yUK3EOlI6gb69NvQwcS_h_eI', // primary
...
export async function fetchWithGeminiFallback(body: any): Promise<any> {
  ...
  for (let i = 0; i < GEMINI_KEYS.length; i++) {
    ...
    try {
      ...
      console.warn(`Key index ${i} failed. Trying next key if available. Error: ${err.message}`);
      lastError = err;
    }
  }
